...
import { actorInputSchema, runSummarySchema, sellerProfileSchema } from './types.js';

const SELLER_URL = 'https://www.n11.com/magaza/gti-bilisim';
...
async function runActor(input: unknown): Promise<{
  ...
  const storageDir = await mkdtemp(path.join(os.tmpdir(), 'seller-intelligence-checklist-'));
...
const sellerRecords = records.filter((record) => !isObject(record) || record.type !== 'RUN_SUMMARY');
const invalidSellerProfiles = sellerRecords
  .map((record, index) => ({ index, result: sellerProfileSchema.safeParse(record) }))
  .filter(({ result }) => !result.success);
const validSellerProfiles = sellerRecords
  .map((record) => sellerProfileSchema.safeParse(record))
  .filter((result) => result.success)
  .map((result) => result.data);

const checks = [
  logCheck('Actor exited successfully', exitCode === 0, `exitCode=${exitCode}`),
  logCheck('At least one seller profile was collected', sellerRecords.length > 0, `records=${sellerRecords.length}`),
  logCheck('All seller profiles match the Zod output schema', invalidSellerProfiles.length === 0, `invalid=${invalidSellerProfiles.length}`),
  ...
  logCheck(
    'Required seller fields are not null',
    validSellerProfiles.every((seller) => Boolean(seller.sellerId && seller.sellerName && seller.sellerUrl && seller.platform)),
  ),
];
